...
//update of the histogram of g(r)
      ...
      walker[igofr+bin] += 2; // +2 in the histo bin, since there are 2 particles
...
// contribution to energy and virial
  ...
  ofstream u ("output.istU", ios::app);
...
void Averages(int iblk) { //Print results for current block
   ...
   const int wd=12;
    ...
    for(int i = igofr ; i < igofr + nbins; i++){
      r = (i-2)*bin_size; //this is in the correct units now
...
//g(r) per blocco finale per ogni bin
    if(iblk == nblk){ 
     for(int i = igofr; i < igofr + nbins; i++){
      r = (i-2)*bin_size;
      Gave << setw(wd) << i - 1 << setw(wd) <<"," << r+bin_size*0.5 << setw(wd) <<"," << glob_av[i]/(double)iblk << setw(wd) <<"," << err_gofr[i] << endl;
     }
    }
    cout << "----------------------------" << endl << endl;
