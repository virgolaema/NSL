...
int main(){ 
  ...
  for(int istep=1; istep <= nstep; ++istep){
     ...
     cout << "We at step " << istep << " of " << nstep << endl;
...
//measurement of g(r) ADDED FOR 7.3
  ...
  cout << "BIN SIZE IS " <<  bin_size << endl << endl;
...
void Measure(){ //Properties measurement
  ...
  cout << "Steps are " << nstep << endl;
  int bin;
  double v, t, vij;
  double dx, dy, dz, dr;

  //reset the hystogram of g(r)
  for (int k=0; k<nbins; k++) walker[k]=0;
...
//cycle over pairs of particles
  for (int i=0; i<npart-1; ++i){
    for (int j=i+1; j<npart; ++j){
      cout << "Steps are " << nstep << " j = " << j << " and i = " << i << " npart is " << npart << endl;
      cout << "coord 1: " << xold[i] << " " << yold[i] << " "<< zold[i] << " " << "coord 2: " << xold[j] << " " << yold[j] << " "<< zold[j] << endl;
      dx = Pbc( xold[i] - xold[j] ); // here I use old configurations [old = r(t)]
      dy = Pbc( yold[i] - yold[j] ); // to be compatible with EKin which uses v(t)
      dz = Pbc( zold[i] - zold[j] ); // => EPot should be computed with r(t)
      dr = dx*dx + dy*dy + dz*dz;
      cout << "coord " << dx << " " << dy << " " << dz << endl;
      dr = sqrt(dr);
      cout << "dr is " << dr << endl;
...
//Kinetic energy
  ...
  Temp.close();
  Etot.close();
  cout << "Measure completed, steps are " << nstep << endl;
  return;
}
