...
void brownianBridge(){
	...
	double new_segment[brownianBridgeReconstructions+2]; //one is the fixed start, one the fixed end
	new_segment[0]=starting_coord;
	new_segment[brownianBridgeReconstructions+1]=ending_coord;
	double previous_position = starting_coord; //then update obv
	for(int i=0;i<brownianBridgeReconstructions;i++){
		int left_reco = brownianBridgeReconstructions-i; //reco still tbd
...
It's quite straightforward. Is it the best one?
*/
...
double variationalLocalEnergy(double val){
	...
	return -(hbar*hbar/(2*mass))*laplacian_psi/psi; //lambda?
}
